...
namespace PlantillaCorreoUS.Registry
{
    public static class ExtensionesAplicacion
    {
        ...
        public static IApplicationBuilder UseConsul(this IApplicationBuilder app)
        {
            ...
            {
                ID = Environment.GetEnvironmentVariable(config.IdServicio) + "-" + Guid.NewGuid().ToString().Substring(0,5),
                // servie name  
                Name = Environment.GetEnvironmentVariable(config.IdServicio),
                Address = $"{uri.Host}",
                Port = uri.Port,
                Checks = new AgentServiceCheck[] {
                    new AgentServiceCheck{
                        ...
                        HTTP = $"http://{uri.Host}:{uri.Port}/us/plantilla/health"
                    }
                }
            };
